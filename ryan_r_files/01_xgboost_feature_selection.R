...
cv_data <- clean_data %>% 
  ...
  mutate(deaths = log(covid_19_deaths)) %>% 
...
data_train2 <- data_train %>% 
  select(-c(deaths, date.x, covid_19_deaths))
...
unlog <- exp(predictions)
mse <- mean((unlog - data_test$covid_19_deaths)^2)
rmse <- sqrt(mse)
print(paste("Mean Squared Error:", mse))
print(paste("Root Mean Squared Error:", rmse))
rss <- sum((unlog - data_test$covid_19_deaths) ^ 2)  ## residual sum of squares
tss <- sum((data_test$covid_19_deaths - mean(data_test$covid_19_deaths)) ^ 2)  ## total sum of squares
rsq <- 1 - rss/tss
print(paste("R-Squared", rsq))
