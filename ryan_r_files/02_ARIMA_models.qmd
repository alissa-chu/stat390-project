...
south_initial_sarima_plot <-ggplot(south_pred2) +
  ...
  scale_x_continuous(limits = c(1023,1136),
                     breaks = c(1023, 1069, 1115),
                     labels = c("Dec 2022", "Jan 2023", "March 2023")) +
  labs(title = "South Region Initial SsARIMA",
       y = NULL,
       x = NULL) +
  scale_color_manual(values = c("dodgerblue", "orange"), labels = c("Actual", "Predicted")) +
  theme_bw() +
...
south_sarima <- arima(south_train_ts, order = c(5, 1, 5),
                           seasonal = list(order = c(1, 1, 1), period = 7))
west_sarima <- arima(west_train_ts, order = c(0, 1, 3),
                     seasonal = list(order = c(1, 1, 1), period = 7))
...
east_arima <- auto.arima(east_train_ts, seasonal = TRUE, max.p = 5, max.q = 5, max.P = 5, max.Q = 5)
